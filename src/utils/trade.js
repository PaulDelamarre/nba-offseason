...
export function evaluateTeam(team, year) {
  ...
  // Salaire entrant maximal autorisé par le matching.
  let maxIncoming;
  if (overApron) {
  ...
  // 1) Salary matching
  const salaryLegal = inSalary <= maxIncoming + EPS;
  const overGap = Math.max(0, inSalary - maxIncoming);
  if (!salaryLegal) {
    errors.push(
      `Matching insuffisant : reprend ${fmt(inSalary)} mais ne peut absorber que ${fmt(maxIncoming)} ` +
      `(sortant ${fmt(outSalary)}). Manque ${fmt(overGap)}.`
    );
  }

  // 2) Hard cap au 1er apron si matching étendu (reprend > envoie, sans cap room)
  const usedExpandedMatching = capRoom === 0 && inSalary > outSalary + EPS && !overApron;
  ...
  return {
    ...
    hardCaps, errors, warnings, tpeGenerated,
  };
}
